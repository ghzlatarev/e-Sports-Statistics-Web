...
namespace ESportStatistics.Web.Tests.Areas.Statistics.TournamentControllerTests
{
    ...
    public class DetailsAction_Should
    {
        ...
        [TestMethod]
        public async Task ThrowApplicationException_WhenPassedTournamentIsNull()
        {
            // Arrange
            Mock<ITournamentService> tournamentServiceMock = new Mock<ITournamentService>();
            Mock<IPDFService> pDFServiceMock = new Mock<IPDFService>();
            ...
            Tournament invalidTournamentResult = null;

            tournamentServiceMock.Setup(mock => mock.FindAsync(validId))
                .Returns(Task.FromResult(invalidTournamentResult));
